...
The AI receives the complete requirement set alongside domain context describing the AIGuidebook system. The prompt asks it to analyze the requirements and propose a full-stack architecture, including frontend framework, backend framework, database, and testing tools, and to justify each choice against specific requirements. For example, NFR-01's 2-second constraint informs database selection, and RE-14's declaration-versus-log comparison motivates a relational model. The output is a component breakdown of API routes, service modules, and data models, which then becomes the input context for all subsequent prompts (**prompt chaining**).
...
After all requirements have passing tests, the AI reviews the full codebase for refactoring opportunities, eliminating duplication, improving cohesion, and verifying that non-functional performance targets are addressed. All existing tests must continue to pass, since the codebase is improved incrementally without breaking validated behavior (**iterative refinement**).

### Architectural Awareness

This strategy produces a full-stack web application with clear separation: a frontend consuming a backend REST API backed by a persistent database. The backend-first, test-driven approach is deliberate, since backend end-to-end tests execute faster than browser-based testing and enable rapid feedback during iterative AI-assisted development. The frontend is developed last, consuming the verified API endpoints.
